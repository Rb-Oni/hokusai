...
<section class="container px-5 lg:px-0 mx-auto py-16">

    <div>
        <div class="flex flex-col lg:flex-row">
            <img src="{{ asset('storage/products/'.$product->img) }}" alt="{{ $product->name }}" class="h-full lg:w-1/3">
            <div class="text-black lg:pl-11 lg:w-45rem">
                <h1 class="text-3xl lg:text-5xl font-bold pb-3">{{ $product->name }} - Tome {{ $product->volume }}</h1>
                <div class="lg:w-10/12">
                    <div class="flex flex-wrap justify-between items-end lg:pb-11">
                        <h2 class="text-xl lg:text-2xl font-semibold hidden lg:block">de <a href="#" class="text-red-500">{{ $product->author }}</a> - {{ date('d M Y', strtotime($product->date)) }}</h2>
                        <a href="#" class="hidden lg:block text-xl font-semibold uppercase text-greenc bg-white px-2 py-1">nouveauté</a>
                    </div>
                    <div class="flex justify-between items-center text-2xl lg:text-4xl lg:pb-11">
                        <a href="#" class="hidden lg:block font-bold text-greenc bg-white px-3 py-2">{{ $product->fv_editor }}</a>
                        <a href="#" class="hidden lg:block font-semibold">{{ $product->category->name }}</a>
                        <span class="hidden lg:block font-semibold text-2xl">12 ans et +</span>
                    </div>
                    <ul class="flex flex-col lg:flex-row justify-between items-end pb-11">
                        <li class="relative">
                        ...
                        <li class="relative pt-5 lg:pt-0">
                    ...
                    <div class="flex flex-col text-2xl font-semibold">
                        <h4 class="hidden lg:block lg:pb-8">
...
<section class="bg-white mb-16">
    ...
    <div x-data="{ openTab: 1 }" class="container mx-auto py-8">
        <ul class="flex flex-col lg:flex-row items-center">
            ...
            <li @click="openTab = 2" :class="{ 'text-greenc':openTab === 2 }" class="py-5 lg:py-0 lg:mx-6">
        ...
        <div class="w-full pt-6 px-5 lg:px-0">
            <div x-show="openTab === 1">
                <span class="font-semibold text-2xl">{{ $product->synopsis }}</span>
            </div>
            <div x-show="openTab === 2" class="grid grid-cols-1 lg:grid-cols-2 font-bold text-greenc text-2xl">
            ...
            <div x-show="openTab === 3" class="grid grid-cols-1 lg:grid-cols-2 font-bold text-greenc text-2xl">
...
<section class="container px-5 lg:px-0 mx-auto pb-16">
